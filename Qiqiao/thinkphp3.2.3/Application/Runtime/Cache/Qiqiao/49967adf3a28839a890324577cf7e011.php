    ...
    <div class="main-content" >
        ...
        <div class="row">
            <div class="col-xs-12 col-md-12 col-lg-6 form-inline pull-right">
                <form class= style="padding:8px 0 " method='post' action="<?php echo U('Plan/titlemanage');?>">
            ...
            <div class="center-block">
                <table class="table table-hover">
                    ...
                    <td>
                        ...
                        <?php if(($v["status"]) == "0"): ?><button class="btn btn-primary btn-danger btn-xs" id="model_<?php echo ($v["id"]); ?>" data-toggle="modal" data-target="#myModal_<?php echo ($v["id"]); ?>">普通团队</button>
                        <?php else: ?>
                        <button class="btn btn-primary btn-danger btn-xs" id="model_<?php echo ($v["id"]); ?>" data-toggle="modal" data-target="#myModal_<?php echo ($v["id"]); ?>">优秀团队</button><?php endif; ?>
                          ...
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                ...
                                <?php if(($v["status"]) == "0"): ?>设为优秀团队
                                <?php else: ?>
                                  撤销优秀团队称号<?php endif; ?>
                                </h4>
                              </div>
                              <div class="modal-body">
                                <?php if(($v["status"]) == "0"): ?>设定优秀团队，前台将在优秀团队板块可见该团队信息
                                <?php else: ?>
                                撤销优秀团队称号，该团队不可再优秀团队板块可见<?php endif; ?>
                              </div>
                    ...
                    <tr>
                    <td colspan="12" class="online" id="none_<?php echo ($v["id"]); ?>">
                        <ul>
                        <li><span class="real_name">团队名称</span><span style="height:100%"><?php echo ($v["team_name"]); ?></span></li>
                        <li><span>团队口号</span><span style="height:100%"><?php echo ($v["team_slogan"]); ?></span></li>
                        </ul>
                        <br>
  
                        <ul>
                        <li><span class="tel">团队简介</span><span style='width:600px;height:100%;margin-bottom: 10px;border:1px solid #EAEAEC;'><?php echo ($v["team_summary"]); ?></span></li>
                        </ul>
                        <br>

                        <?php if($v["team_pic"] == null): ?><ul>
                        ...
                        <?php else: ?>
                        <ul>
                        <li><span>团队照片</span><span style='width:600px;height:100%;margin-bottom: 10px;border:1px solid #EAEAEC;'><img src='/thinkphp3.2.3/Public/Home/image/team/<?php echo ($v["team_pic"]); ?>'  width='300px' height='100%' /></span></li>
                        </ul>
                        <br><?php endif; ?>
						...
						<li><span>团队成员</span><span style='width:600px;height:100%;margin-bottom: 10px;border:1px solid #EAEAEC;'>
							<?php if(is_array($arr[$v['id']])): $i = 0; $__LIST__ = $arr[$v['id']];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><br>
                        ...
                        <ul>
                        	<li><span>团队作品</span><span style="width:600px;height:100%;margin-bottom: 10px;border:1px solid #EAEAEC;">
                        		<?php if(is_array($array[$v['id']])): $i = 0; $__LIST__ = $array[$v['id']];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><br>
                        				<h5><?php echo ($val["video_title"]); ?>------<?php echo (date('Y-m-d',$val["vidup_time"])); ?> </h5>
                        			<br><?php endforeach; endif; else: echo "" ;endif; ?>
...
function lock(id,num){
       $.ajax({
           url:'<?php echo U('Plan/lock_team');?>',
           type:'post',
           dataType:'json',
           data:{"id":id,"num":num},
           success:function(data){
              if(data=='1'){
                window.location.href="<?php echo U('Plan/titlemanage');?>";
              }
              else{
