    ...
    <div class="main-content" >
        ...
        <div class="row">
            <div class="col-xs-12 col-md-12 col-lg-6 form-inline pull-right">
                    ...
                    <form class= style="padding:8px 0 " action="<?php echo U('Information/manage');?>" method="post" id="information_form">
                    ...
                    <select  name="block_select" class="pull-right text-center" style="border-radius:4px;width:120px;height:34px;margin:5px 3px">
                        ...
                        <?php if(is_array($block_select)): $i = 0; $__LIST__ = $block_select;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["blockname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                    </form>
            </div>
        </div>
        <div class="row">
            <div class="center-block">
                <table class="table table-hover">
                    ...
                    <td>
                        <button class="btn btn-primary btn-xs information" onclick="display_information(<?php echo ($v["id"]); ?>)">资讯</button>
                        <?php if(($v["is_send"]) == "0"): ?><button class="btn btn-primary btn-danger btn-xs" id="model_<?php echo ($v["id"]); ?>" data-toggle="modal" data-target="#myModal_<?php echo ($v["id"]); ?>">发布</button>
                        <?php else: ?>
                        <button class="btn btn-primary btn-danger btn-xs" id="model_<?php echo ($v["id"]); ?>" data-toggle="modal" data-target="#myModal_<?php echo ($v["id"]); ?>">撤销</button><?php endif; ?>                        
                        <!-- 模态框（Modal） -->
                            ...
                            <div class="modal-content">
                              <div class="modal-header">
                                ...
                                <h4 class="modal-title" id="myModalLabel">
                                  <?php if(($v["is_send"]) == "0"): ?>发布资讯
                                  <?php else: ?>
                                  撤销资讯<?php endif; ?>
                                </h4>
                              </div>
                              <div class="modal-body">
                                <?php if(($v["is_send"]) == "0"): ?>发布资讯后，将在前台可见
                                <?php else: ?>
                                撤销资讯后，将在前台不可见<?php endif; ?>
                              </div>
                              <div class="modal-footer">
                                <button type="button" class="btn btn-default" data-dismiss="modal">关闭
                                </button>

                                <?php if(($v["is_send"]) == "0"): ?><button type="button" class="btn btn-primary" onclick="
...
<script src="/thinkphp3.2.3/Public/js/scripts.js"></script>
    ...
    function display_information(element){
        window.location.href="<?php echo U('Information/display_information');?>"+'/id/'+element;
    }


    $("#add_information").click(function(){
        window.location.href="<?php echo U('Information/add');?>";
    });

    function lock(element,num){
        $.ajax({
           url:'<?php echo U('Information/change');?>',
           type:'post',
           dataType:'json',
           ...
           success:function(data){
                //console.log(data);
              if(data == '1'){
                window.location.href="<?php echo U('Information/manage');?>";
              }
              else{
